...
class ConvFeatureStack(nn.Module):
    def __init__(self, n_layers: int, n_hidden: int, channels: int, n_scales: int):
        super().__init__()
        in_ch = channels
        layers = []
        for _ in range(n_layers):
            layers.append(MultiScaleConv2d(in_ch, n_hidden, n_scales))
            in_ch = n_hidden
...
class ConvDenseDenoiseNet(nn.Module):
    def __init__(
        ...
        self.conv_stack = ConvFeatureStack(n_layers_conv, n_hidden_conv, channels, n_scales)

        if n_hidden_dense_lower > 0 and n_layers_dense_lower > 0:
            n_flat_in = channels * spatial_width ** 2
        ...
        n_out = channels * 2 * n_temporal_basis
        ...
        last_linear = next(m for m in reversed(self.dense_upper.net) if isinstance(m, nn.Linear))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.dense_lower is not None:
            b = x.shape[0]
            flat = x.reshape(b, self.channels * self.spatial_width ** 2)
            dense_feat = self.dense_lower(flat).reshape(
                b, self.spatial_width, self.spatial_width, self.n_hidden_dense_lower_output
            )
            feat = torch.cat([
                conv_feat  / math.sqrt(self.n_hidden_conv),
                dense_feat / math.sqrt(self.n_hidden_dense_lower_output),
            ], dim=-1)
