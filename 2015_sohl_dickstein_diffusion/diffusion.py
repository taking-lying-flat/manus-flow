...
from denoise import ConvDenseDenoiseNet
...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        schedule: Optional[LearnableBetaSchedule] = None,
    ):
        super().__init__()
        self.config = config
    ...
    def p_mean_log_variance(
        ...
        model_var = torch.sigmoid(
            ...
            + var_coeff / math.sqrt(float(self.T))
    ...
    def kl_elbo_loss(
        ...
        neg_log_bound = kl * float(self.T) + entropy_start - entropy_end + const
    ...
    def forward(self, x0: torch.Tensor) -> torch.Tensor:
        ...
        return loss / float(self.n_t_per_minibatch)
...
class DiffusionModel(GaussianDiffusion):
    ...
    def __init__(self, spatial_width: int, n_colors: int, uniform_noise: float = 0.0, **cfg_overrides):
        super().__init__(DiffusionConfig(
            image_size=int(spatial_width),
            channels=int(n_colors),
