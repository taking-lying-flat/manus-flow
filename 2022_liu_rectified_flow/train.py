...
UNET_CONFIGS = {
    "afhq":       {"dim": 128, "dim_mults": (1, 2, 4, 8)},       # 256 → 16
    "celeba-hq":  {"dim": 128, "dim_mults": (1, 2, 4, 8)},       # 64 → 4
    "cifar-10":   {"dim": 128, "dim_mults": (1, 2, 4)},          # 32 → 4
    "bedroom":    {"dim": 128, "dim_mults": (1, 2, 4, 8)},       # 128 → 8
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Rectified Flow.")
    parser.add_argument("--dataset", default="celeba-hq", choices=["afhq", "celeba-hq", "cifar-10", "bedroom"])
    ...
    parser.add_argument("--loss-fn", default="mse", choices=["mse", "pseudo_huber", "pseudo_huber_with_lpips"])
    parser.add_argument("--timestep-sampler", default="uniform", choices=["uniform", "logit_normal", "cosmap", "mode"])
    ...
    parser.add_argument("--ode-method", default="midpoint", choices=["euler", "midpoint", "rk4", "dopri5"])
    ...
    parser.add_argument("--log-interval", default=25, type=int, help="steps between loss prints")
    parser.add_argument("--ckpt-interval", default=500, type=int, help="steps between checkpoint saves (overwrites)")
    return parser.parse_args()


@torch.no_grad()
def save_samples(rf: RectifiedFlow, data_shape: tuple, step: int, output_dir: Path) -> None:
    sample_dir = output_dir / "samples"
    sample_dir.mkdir(parents=True, exist_ok=True)

    rf.eval()
    sampled = rf.sample(batch_size=64, steps=16, data_shape=data_shape).cpu()
    rf.train()
...
def train(args: argparse.Namespace) -> None:
    ...
    unet_cfg = UNET_CONFIGS[args.dataset]
    model = Unet(dim=unet_cfg["dim"], dim_mults=unet_cfg["dim_mults"], channels=3).to(device)
    ...
    rf = RectifiedFlow(
        ...
        timestep_sampler_kwargs=dict(m=args.ts_logit_m, s=args.ts_logit_s, mode_s=args.ts_mode_s),
    ...
    lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.iterations, eta_min=0.0)
    ...
    for step in range(1, args.iterations + 1):
        ...
        if step % args.ckpt_interval == 0 or step == args.iterations:
            ...
            save_samples(rf, data_shape, step, output_dir)
...
def main() -> None:
    args = parse_args()
    image_size = IMAGE_SIZE[args.dataset]
    print(f"Training Rectified Flow on {args.dataset} ({image_size}x{image_size})  device={device}")
    train(args)
